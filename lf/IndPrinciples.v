...
Definition branch_case (P : booltree_property_type) : Prop :=
  forall b : bool, forall t1 : booltree, P t1 ->
...
(** **** Exercise: 1 star, standard, optional (plus_explicit_prop)
    ...
    defined proposition.  *)
Definition P_add_assoc (n m p : nat) :=
  n + (m + p) = (n + m) + p.

Theorem and_assoc'' : forall n m p : nat,
  P_add_assoc n m p.
Proof.
  intros n m p. apply nat_ind.
  - unfold P_add_assoc. rewrite -> add_0_r. rewrite -> add_0_r. reflexivity.
  - unfold P_add_assoc. intros n' IHn'. rewrite <- plus_n_Sm.
    rewrite <- plus_n_Sm. rewrite IHn'. rewrite plus_n_Sm. reflexivity.
Qed.

Definition P_add_comm (n m : nat) :=
  n + m = m + n.
...
Proof.
  ...
  - unfold P_add_comm. simpl. rewrite -> add_0_r. reflexivity.
  - unfold P_add_comm. intros n' IHn'. rewrite <- plus_n_Sm. rewrite IHn'.
...
Definition better_t_tree_ind_type : Prop :=
  forall (X : Type) (P : t_tree X -> Prop),
    P t_leaf ->
    (forall v : X, forall l : t_tree X, P l -> forall r : t_tree X, P r ->
      P (t_branch (l, v, r))) ->
...
Fixpoint build_proof_t_tree
  (X : Type) (P : t_tree X -> Prop) (Pleaf : P t_leaf)
  (Pbranch : forall (v : X) (l : t_tree X), P l -> forall r : t_tree X, P r ->
    P (t_branch (l, v, r))) (t : t_tree X) : P t :=
  match t with
  | t_leaf => Pleaf
  | t_branch (l, v, r) =>
    Pbranch v l (build_proof_t_tree X P Pleaf Pbranch l) r
      (build_proof_t_tree X P Pleaf Pbranch r)
  end.
