...
Proof.
  unfold partial_function. intros x y1 y2 H1. inversion H1.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion Hmo.
    ...
    + apply le_S in Hnm.
      apply le_trans with (a := (S n)) (b := (S m)) (c := (S o')). apply Hnm.
      apply Hmo.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (le_Sn_n_inf)
    ...
    Proof: *)
    (* Let's prove by induction on n. For n = 0, ~ (S 0 <= 0) holds.
       Suppose that the theorem holds for n'. That is, ~ (S n' <= n').
       By le_S_n, ~ (S (S n') <= S n') so the theorem holds.
    [] *)
...
Theorem le_not_symmetric :
  ~ (symmetric le).
Proof.
  unfold symmetric. intros Hcontra. assert (0 <= 1) as H.
  { apply le_S. apply le_n. } apply Hcontra in H. inversion H.
Qed.
(** [] *)
...
Theorem le_antisymmetric :
  antisymmetric le.
Proof.
  unfold antisymmetric. intros a b Hab Hba. inversion Hab.
  - reflexivity.
  - inversion Hba.
    + rewrite H0. rewrite H1. reflexivity.
    + rewrite H1 in Hab. rewrite <- H2 in Hab. apply le_Sn_n in Hab.
      destruct Hab.
Qed.
(** [] *)
...
Theorem le_step : forall n m p,
  ...
  n <= p.
Proof.
  unfold lt. intros n m p H_Sn_m H_m_Sp. apply le_S_n.
  apply le_trans with (a := (S n)) (b := m) (c := (S p)). apply H_Sn_m.
  apply H_m_Sp.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. apply H.
  - intros H. apply IHHxy in H. apply rt1n_trans with y. apply Hxy. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction H.
    + apply rt_refl.
    + apply rt_trans with y. apply rt_step. apply Hxy.
